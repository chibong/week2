...
def rent_for(title)
  puts "The Rent for #{title} is #{MONOPOLY_GAME[:deeds][title][:rent]}"
  return MONOPOLY_GAME[:deeds][title][:rent]
end

def price_for(title)
  return MONOPOLY_GAME[:deeds][title][:price]
end
...
def pick_free_parking_card
  return "Advance to Free Parking"
end
